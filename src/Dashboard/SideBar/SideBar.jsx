...
const SideBar = () => {
  ...
  const menuItems = [
    { label: "Dashboard", icon: <DashboardIcon className="fill-[var(--primary-color)]" />, active: true },
  ...
  return (
    <div className="relative flex flex-col h-full">
      <div className="max-w-[50px] overflow-hidden mx-auto xl:m-0 xl:max-w-full ">
        <img src={logo} alt="logo" className="w-full max-w-[162px] mb-[30px]  object-cover w-full h-full object-left xl:object-none xl:h-auto" />
      </div>
      ...
      <div className="h-[55px] mb-[5px] w-full flex sm:flex-col xl:flex-row items-center gap-3 px-3 py-2 rounded-xl sm:text-[10px] xl:text-sm text-gray-600 group hover:bg-gray-100 transition cursor-pointer"
      ...
      >
       ...
       {isDark ? (
        ...
        <button className={`group bg-[#f2f2f2] group-hover:bg-gray-200 rounded-full px-3 py-2 relative`} onClick={() => setIsDark(!isDark)}>
              <span className={` transition-all duration-200 ease-linear bg-[var(--primary-color)] block absolute w-[22px] h-[23px] rounded-full top-1/2 left-1/2 
              ${isDark ? "-translate-x-full -translate-y-1/2" : "-translate-x-full -translate-y-1/2 left-[87%] " }`}></span>
                     <span className="flex items-center gap-2 z-2 relative " onClick={() => setIsDark(!isDark)}>
                      <SunIcon
                      className={`${isDark ? " " : " fill-[#333] stroke-[#333]"}`}
                      />
                      <MoonIcon
                        className={`${!isDark ? "stroke-[#fff] " : " "}`}
                      />
